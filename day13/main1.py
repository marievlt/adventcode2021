...
def fold_grid(grid, fold):
    ...
    print ("grid dim = ", len(grid), len(grid[0]))
    print ("dir, value = ", direction, value)
    if (direction == 'y'):
        height      = min(value, len(grid)-value)
        if (len(grid)%2 != 0):
            first       = grid[value-height:value, :]
            second      = np.flipud(grid[value+1:value+height+1 , :])
        else:
            first       = grid[value-height:value, :]
            second      = np.flipud(grid[value:value+height , :])


    if (direction == 'x'):
        width       = min(value, len(grid[0])-value)
        if (len(grid[0])%2 != 0):
            first       = grid[:, value-width:value]
            second      = np.fliplr(grid[:, value+1:value+width+1])
        else:
            first       = grid[:, value-width:value]
            second      = np.fliplr(grid[:, value:value+width])
