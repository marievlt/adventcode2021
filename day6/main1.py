...
def main():
    ...
    lanternfishes = [int(i) for i in lines[0].strip().split(',')]

    for i in range(N):
        lanternfishes = next_day(lanternfishes)

    print ("Number of lanternfishes = ", len(lanternfishes))

def next_day(lanternfishes):
    for i in range(len(lanternfishes)):
        if (lanternfishes[i] > 8):
