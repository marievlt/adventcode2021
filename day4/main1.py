...
def main():
    ...
    lines.pop(0)
    grids_nbs = int(len(lines)/6)

    grids = np.zeros((grids_nbs, 5, 5), dtype=int)
    hits = np.zeros((grids_nbs, 5, 5), dtype=int)

    for i in range(grids_nbs):
        for j in range (5):
            values = (lines[6*i+1+j].strip()).split()
            for k in range(5):
    ...
    for number in choosen_nbs:
        ...
        for i in range(len(find_position[0])):
            ...
            if (is_bingo(hits[find_position[0][i]]) == True):
                win_grid = grids[find_position[0][i]]
                win_hits = hits[find_position[0][i]]
                win_number = int(number)
                result = score(win_grid, win_hits, win_number)
                print (result)
                return result
...
def is_bingo(grid):
    ...
    for i in range(5):
        if (sum_line[i] == 5 or sum_column[i] == 5):
            return True
    return False
